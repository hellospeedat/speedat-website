    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      ...
      <Card className="w-full max-w-md shadow-2xl border-0">
        <CardHeader className="space-y-1 flex flex-col items-center text-center pb-8 pt-10">
          ...
          <CardTitle className="text-2xl font-bold text-secondary">Speedat Admin</CardTitle>
          <p className="text-sm text-gray-500">Enter your credentials to access the dashboard</p>
        </CardHeader>
        <CardContent className="pb-10 px-8">
          {error && (
            <div className="mb-6 p-3 bg-red-50 text-red-700 text-sm rounded-md border border-red-200 text-center">
              {error}
            </div>
          )}
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <Button 
              type="submit" 
              disabled={loading} 
              className="w-full h-12 bg-secondary hover:bg-secondary/90 text-white mt-4"
            >
